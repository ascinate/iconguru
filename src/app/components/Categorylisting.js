...
export default function CategoryListing() {
  ...
  // 👇 set initial filter to slug category
  useEffect(() => {
    if (category) {
      setFilters((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    const fetchIcons = async () => {
      ...
      try {
        ...
        if (searchKeyword) {
          query.append("search", searchKeyword);
        }
  ...
  return (
    <>
      <Head>
        <title>{category} Icons - IconsGuru</title>
        <meta name="description" content={`Explore ${category} icons with filters`} />
          ...
          <div className="row">
            ...
            <div className="col-lg-9 ps-lg-4">
              <div className="main-divs g-col-6">
                ...
                <p>Showing <strong className="serch-data">{totalIcons}</strong> Icons</p>
                ...
                <div className="tabsd_divs d-inline-block w-100 mt-2">
                  <ul className="nav nav-tabs" role="tablist">
                    <li className="nav-item" role="presentation">
                      <button className="nav-link active ps-0" data-bs-toggle="tab" data-bs-target="#icons1" type="button" role="tab">Icons</button>
                    </li>
                    <li className="nav-item" role="presentation">
                      <button className="nav-link" data-bs-toggle="tab" data-bs-target="#interface" type="button" role="tab">Interface Icons</button>
                    </li>
                    <li className="nav-item" role="presentation">
                      <button className="nav-link" data-bs-toggle="tab" data-bs-target="#animated" type="button" role="tab">Animated Icons</button>
                    </li>
                  </ul>
                  ...
                  <div className="tab-content" id="myTabContent">
                    <div className="tab-pane fade show active" id="icons1" role="tabpanel">
                      <div className="row row-cols-1 row-cols-lg-6 gy-2 g-lg-2 mt-0">
                        {isLoading ? (
                          ...
                          icons.map((icon) => (
                            <article key={icon.Id} className="svg-item col position-relative">
                              <Link href={`/search/${icon.Id}`} className="btn icons-list p-0">
                                <span dangerouslySetInnerHTML={{ __html: icon.icon_svg }} />
                              </Link>
                              <div className="hover-divs">
                                <button type="button" className="btn btn-adds">
                                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="rgba(67,67,67,1)">
                          ...
                          <div className="col"><p>No icons found.</p></div>
                        )}
                      </div>
                    ...
                    <div className="tab-pane fade" id="interface" role="tabpanel"></div>
                    <div className="tab-pane fade" id="animated" role="tabpanel"></div>
                  </div>
                </div>
              </div>
            </div>
